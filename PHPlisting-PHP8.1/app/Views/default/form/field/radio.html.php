<?php
    ...
    $view->attributes->add('type', 'radio');
    ...
    foreach ((array) ($view->getOptions() ?? []) as $value => $title) {
        ...
        if ((string) $value == $view->value) {
            $view->attributes->add('checked', true);
        }

        $view->attributes->add('id', $id . $count);
?>
<div class="custom-control custom-radio pb-2">
    <input<?php echo $view->attributes; ?>>
    <label class="custom-control-label pl-2" for="<?php echo e($view->attributes->id); ?>"><?php echo e($title); ?></label>
</div>
<?php 
        $count++;
    }
